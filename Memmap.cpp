...
of hardcoding it to 20 everywhere in the Borer.
...
4) Borer now ships as standard with VC 5 workspace files
...
BOOL CMemMapFile::MapFile(const CString& sFilename, BOOL bReadOnly, 
                          DWORD dwShareMode, BOOL bAppendNull, 
                          BOOL bNamed, BOOL bGrowable)
{
  ...
  if ((unsigned int)m_dwLength == 0xFFFFFFFF)
  {
...
BOOL CMemMapFile::MapExistingMemory(const CString& sName, DWORD dwBytes, BOOL bReadOnly)
{
  ...
  //Map the view
  m_lpData = (unsigned char*)MapViewOfFile(m_hMapping, dwDesiredAccess, 0, 0, m_dwLength);
...
BOOL CMemMapFile::MapHandle(HANDLE hHandle)
{
  ...
  m_lpData = (unsigned char*)MapViewOfFile(m_hMapping, dwDesiredAccess, 0, 0, 0);
