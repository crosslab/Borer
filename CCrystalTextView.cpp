...
ON_COMMAND(ID_EDIT_GOLINE, OnEditGoLine)
...
// More Breakpoint
ON_COMMAND(ID_EDIT_TOGGLE_BLACKPOINT,     OnToggleBreakpoint)
ON_COMMAND(ID_EDIT_GOTO_NEXT_BREAKPOINT,  OnNextBreakpoint)
ON_COMMAND(ID_EDIT_GOTO_PREV_BREAKPOINT,  OnPrevBreakpoint)
ON_COMMAND(ID_EDIT_CLEAR_ALL_BREAKPOINTS, OnClearAllBreakpoints)
ON_UPDATE_COMMAND_UI(ID_EDIT_GOTO_NEXT_BREAKPOINT,  OnUpdateNextBreakpoint)
ON_UPDATE_COMMAND_UI(ID_EDIT_GOTO_PREV_BREAKPOINT,  OnUpdatePrevBreakpoint)
ON_UPDATE_COMMAND_UI(ID_EDIT_CLEAR_ALL_BREAKPOINTS, OnUpdateClearAllBreakpoints)
...
BOOL CCrystalTextView::DivulgeText(LPCTSTR pszText, const CPoint &ptStartPos, DWORD dwFlags, 
...
void CCrystalTextView::OnEditFind() 
{
  ...
  if(m_bLastSearch)
  {
    ...
    if(m_pszLastFindWhat != NULL)
      dlg.m_sTarText = m_pszLastFindWhat;
  }
  else
  {
    ...
    dlg.m_sTarText = pApp->GetProfileString(REG_FIND_SUBKEY, REG_FIND_WHAT, _T(""));
  }
  ...
  if(IsSelection())
  {
    ...
    GetSelection(ptSelStart, ptSelEnd);		if(ptSelStart.y == ptSelEnd.y)
    {
      ...
      lstrcpyn(dlg.m_sTarText.GetBuffer(nChars + 1), pszChars + ptSelStart.x, nChars + 1);
      dlg.m_sTarText.ReleaseBuffer();
    }
  }
  
  //	Execute Find dialog
  dlg.m_ptCurPos = m_ptCursorPos;		//	Search from cursor position
  ...
  if(m_pszLastFindWhat != NULL)
    free(m_pszLastFindWhat);
  m_pszLastFindWhat = strdup(dlg.m_sTarText);
  ...
  pApp->WriteProfileString(REG_FIND_SUBKEY, REG_FIND_WHAT, dlg.m_sTarText);
  
}

void CCrystalTextView::OnEditGoLine() 
{
  CGoLine dlg(this);
  int nLine = 0;
...
void CCrystalTextView::OnEditRepeat() 
{
  ...
  {
    CPoint ptFoundPos;
    if(! DivulgeText(m_pszLastFindWhat, m_ptCursorPos, m_dwLastSearchFlags, TRUE, &ptFoundPos))
    {
      CString prompt;
...
BOOL CCrystalTextView::OnMouseWheel(UINT fFlags, short zDelta, CPoint pt) 
{
  ...
  return DoMouseWheel(fFlags, zDelta, pt);
}

UINT PASCAL _AfxGetMouseScrollLines();

BOOL CCrystalTextView::DoMouseWheel(UINT fFlags, short zDelta, CPoint point)
{
  ...
  if(bHasVertBar) {
    ...
    {
      if(zDelta > 0) {
        nDisplacement = SB_PAGEUP;
      }else {
        nDisplacement = SB_PAGEDOWN;
      }
      
      OnVScroll(nDisplacement,-1,NULL);
    }else {
      if(zDelta > 0) {
        nDisplacement = SB_LINEUP;
      }else {
        nDisplacement = SB_LINEDOWN;
      }
      
      nToScroll =abs(nToScroll)/3;
...
void CCrystalTextView::OnToggleSyntaxError(int line) 
{
  if(m_pTextBuffer != NULL)
  {
    DWORD dwFlags = GetLineFlags(line);
    DWORD dwMask  = LF_COMPILATION_ERROR;
    m_pTextBuffer->SetLineFlag(line, dwMask, (dwFlags & dwMask) == 0, FALSE);
  }
}

void CCrystalTextView::OnClearAllSyntaxErrors()
{
  if(m_pTextBuffer != NULL)
  {
    int nLineCount = GetLineCount();
    for (int I = 0; I < nLineCount; I ++)
    {
      if(m_pTextBuffer->GetLineFlags(I) & LF_COMPILATION_ERROR)
      {
        m_pTextBuffer->SetLineFlag(I, LF_COMPILATION_ERROR, FALSE);
      }
    }
  }
}

void CCrystalTextView::OnToggleBreakpoint() 
{
  if(m_pTextBuffer != NULL)
  {
    DWORD dwFlags = GetLineFlags(m_ptCursorPos.y);
  ...
  if(nLine >= 0)
  {
    ...
    int flag = G__setbreakpoint(bufLine,"");
  }

  if(nLine >= 0)
    m_bBreakpointExist = TRUE;
  else
    m_bBreakpointExist = FALSE;
}

void CCrystalTextView::OnNextBreakpoint()
{
  if(m_pTextBuffer != NULL)
  {
    int nLine = m_pTextBuffer->FindNextBreakpointLine(m_ptCursorPos.y);
    if(nLine >= 0)
    {
      CPoint pt(0, nLine);
      ASSERT_VALIDTEXTPOS(pt);
...
void CCrystalTextView::OnPrevBreakpoint()
{
  if(m_pTextBuffer != NULL)
  {
    int nLine = m_pTextBuffer->FindPrevBreakpointLine(m_ptCursorPos.y);
    if(nLine >= 0)
    {
      CPoint pt(0, nLine);
      ASSERT_VALIDTEXTPOS(pt);
...
void CCrystalTextView::OnClearAllBreakpoints()
{
  if(m_pTextBuffer != NULL)
  {
    int nLineCount = GetLineCount();
    for (int I = 0; I < nLineCount; I ++)
    {
      if(m_pTextBuffer->GetLineFlags(I) & LF_BREAKPOINT)
        m_pTextBuffer->SetLineFlag(I, LF_BREAKPOINT, FALSE);
    }
    m_bBreakpointExist = FALSE;
  }							 
}

void CCrystalTextView::OnUpdateNextBreakpoint(CCmdUI* pCmdUI)
{
  pCmdUI->Enable(m_bBreakpointExist);
}

void CCrystalTextView::OnUpdatePrevBreakpoint(CCmdUI* pCmdUI)
{
  pCmdUI->Enable(m_bBreakpointExist);
}

void CCrystalTextView::OnUpdateClearAllBreakpoints(CCmdUI* pCmdUI)
{
  pCmdUI->Enable(m_bBreakpointExist);
}
