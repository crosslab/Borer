...
BOOL CCrystalTextBuffer::InitNew(int nCrlfStyle /*= CRLF_STYLE_DOS*/)
{
  ...
  RedrawViews(NULL, NULL, UPDATE_RESET);
  return TRUE;
}
...
void CCrystalTextBuffer::SetLineFlag(int nLine, DWORD dwFlag, BOOL bSet, BOOL bRemoveFromPreviousLine /*= TRUE*/)
{
  ...
  if (m_aLines[nLine].m_dwFlags != dwNewFlags)
  {
    if (bRemoveFromPreviousLine)
    {
      ...
      if (bSet)
      {
        if (nPrevLine >= 0)
        {
          ...
          RedrawViews(NULL, NULL, UPDATE_SINGLELINE | UPDATE_FLAGSONLY, nPrevLine);
        }
      }
      else
      {
    ...
    RedrawViews(NULL, NULL, UPDATE_SINGLELINE | UPDATE_FLAGSONLY, nLine);
  }
}
...
void CCrystalTextBuffer::RedrawViews(CCrystalTextView *pSource, CUpdateContext *pContext, DWORD dwUpdateFlags, int nLineIndex /*= -1*/)
{
  POSITION  pos =m_lpViews.GetHeadPosition();
  CCrystalTextView *pView;
  
  while(pos != NULL) {
    pView= m_lpViews.GetNext(pos);
    
    if(dwUpdateFlags & UPDATE_RESET) {
      pView->Invalidate();
    }
...
BOOL CCrystalTextBuffer::DeleteInternalText(CCrystalTextView *pSource, int nStartLine, int nStartChar, int nEndLine, int nEndChar)
{
  ASSERT(m_bInit);	//	Text buffer not yet initialized.
  //	You must call InitNew() or LoadFromFile() first!
  ASSERT(nStartLine >= 0 && nStartLine < m_aLines.GetSize());
  ...
  if (nStartLine == nEndLine)
  {
    ...
    RedrawViews(pSource, &context, UPDATE_SINGLELINE | UPDATE_HORZRANGE, nStartLine);
  }
  else
  {
    ...
    RedrawViews(pSource, &context, UPDATE_HORZRANGE | UPDATE_VERTRANGE, nStartLine);
  }
...
BOOL CCrystalTextBuffer::InsertInternalText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar)
{
  ASSERT(m_bInit);	//	Text buffer not yet initialized.
  //	You must call InitNew() or LoadFromFile() first!
  ...
  if (bNewLines)
    RedrawViews(pSource, &context, UPDATE_HORZRANGE | UPDATE_VERTRANGE, nLine);
  else
    RedrawViews(pSource, &context, UPDATE_SINGLELINE | UPDATE_HORZRANGE, nLine);
...
#endif
      VERIFY(DeleteInternalText(NULL, ur.m_ptStartPos.y, ur.m_ptStartPos.x, ur.m_ptEndPos.y, ur.m_ptEndPos.x));
      ptCursorPos = ur.m_ptStartPos;
    }
    else
    {
      int nEndLine, nEndChar;
      VERIFY(InsertInternalText(NULL, ur.m_ptStartPos.y, ur.m_ptStartPos.x, ur.GetText(), nEndLine, nEndChar));
#ifdef _ADVANCED_BUGCHECK
...
{
  if (! InsertInternalText(pSource, nLine, nPos, pszText, nEndLine, nEndChar))
    return FALSE;
                                    ...
                                    int nEndLine, int nEndChar, int nAction)
{
  ...
  if (! DeleteInternalText(pSource, nStartLine, nStartChar, nEndLine, nEndChar))
    return FALSE;
...
int CCrystalTextBuffer::FindNextBreakpointLine(int nCurrentLine)
{
  BOOL bWrapIt = TRUE;
  DWORD dwFlags = GetLineFlags(nCurrentLine);
  if ((dwFlags & LF_BREAKPOINT) != 0)
    nCurrentLine++;
  ...
  { 
    while (nCurrentLine < nSize)
    {
      if ((m_aLines[nCurrentLine].m_dwFlags & LF_BREAKPOINT) != 0)
        return nCurrentLine;
      // Keep going
      nCurrentLine++;
    }
...
int CCrystalTextBuffer::FindPrevBreakpointLine(int nCurrentLine)
{
  BOOL bWrapIt = TRUE;
  DWORD dwFlags = GetLineFlags(nCurrentLine);
  if ((dwFlags & LF_BREAKPOINT) != 0)
