...
#define LF_BREAKPOINTMARK(id)		(LF_BREAKPOINT << id)
#define LF_EXECUTIONMARK(id)		(LF_EXECUTION << id)
...
#pragma pack(pop)
  ...
  //	Implementation
  BOOL InsertInternalText(CCrystalTextView *pSource, int nLine, int nPos, LPCTSTR pszText, int &nEndLine, int &nEndChar);
  BOOL DeleteInternalText(CCrystalTextView *pSource, int nStartLine, int nStartPos, int nEndLine, int nEndPos);
  ...
  public:
    ...
    void RedrawViews(CCrystalTextView *pSource, CUpdateContext *pContext,
    ...
    int FindNextBreakpointLine(int nCurrentLine = 0);
    int FindPrevBreakpointLine(int nCurrentLine = 0);
