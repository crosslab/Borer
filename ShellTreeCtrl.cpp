...
void CShellTreeCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) //？？？？
{
  // get clicked item
  ...
  pData->pParentFolder->GetDisplayNameOf(pidlRel,SHGDN_NORMAL|SHGDN_FORPARSING,
    str.GetPointer(pidlRel));
  strPathName = str;	// copy to string
		
  CFileFind dclkFinder;//判断文件是否存在
  dclkFinder.FindFile(strPathName, 0 );
  
  BOOL bWorking = dclkFinder.FindFile(strPathName, 0 );
  if(!bWorking)
  {
    goto LabelLButtonDblClk;
  }
  
  bWorking = dclkFinder.FindNextFile();
  
  if (dclkFinder.IsDirectory())
  {
    goto LabelLButtonDblClk;
  }
  ...
  CBorerDoc  *pMyDoc;
 	...
 	pMyFrm=((CMainFrame*)AfxGetMainWnd())->GetActiveFrame();
  if(!pMyFrm) {
    goto LabelLButtonDblClk; ;
  }
  
  pMyView=pMyFrm->GetActiveView();
  if(!pMyView) {
    goto LabelLButtonDblClk; ;
  }
  
  pMyDoc=(CBorerDoc*) pMyView->GetDocument();
  
  pMyDoc->m_xTextBuffer.FreeAll();
  pMyDoc->m_xTextBuffer.LoadFromFile(strPathName);
  pMyDoc->SetPathName(strPathName);
  
LabelLButtonDblClk:CWaitingTreeCtrl::OnLButtonDblClk(nFlags, point);
}

BOOL CShellTreeCtrl::PreCreateWindow(CREATESTRUCT& cs) //？？？？
{
  cs.style |= TVS_HASBUTTONS | TVS_LINESATROOT | TVS_HASLINES;	
  return CWaitingTreeCtrl::PreCreateWindow(cs);
}
