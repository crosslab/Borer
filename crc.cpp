...
unsigned int 
crc32(const unsigned char *address, unsigned int size, unsigned int crc)
{
#if 0
...
static const unsigned int crctableA[16] =
{
  ...
  static const unsigned int crctableB[16] =
  {
    ...
    unsigned short 
      crc16(const unsigned char *address, unsigned int size,
      unsigned short crc)
    {
      for (; (size > 0); size--)
      {
        ...
        data ^= (unsigned char)crc;            /* EOR data with current CRC value */
        crc =(unsigned short) ((crctableA[(data & 0xF0) >> 4] ^ crctableB[data & 0x0F]) ^
          (crc >> 8));
      }
