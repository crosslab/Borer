...
2. Fixed leaks which can occur in Borer app when an exception is thrown
...
void CSerialPort::Dump(CDumpContext& dc) const
{
  ...
  dc << _T("port_hComm = ") << port_hComm << _T("\n");
  dc << _T("m_bOverlapped = ") << m_bOverlapped;
}
#endif

void CSerialPort::PJNOpen()
{
}

void CSerialPort::PJNClose()
{
  if (IsEngrossed())
  {
    BOOL bSuccess = CloseHandle(port_hComm);
    port_hComm = INVALID_HANDLE_VALUE;
...
void CSerialPort::PJNAttach(HANDLE hComm)
{
  PJNClose();
  port_hComm = hComm;  
}

HANDLE CSerialPort::PJNDetach()
{
  HANDLE hrVal = port_hComm;
  port_hComm = INVALID_HANDLE_VALUE;
  return hrVal;
}

DWORD CSerialPort::PJNRead(void* lpBuf, DWORD dwCount)
{
  ASSERT(IsEngrossed());
  ASSERT(!m_bOverlapped);
  
  DWORD dwBytesRead = 0;
  if (!ReadFile(port_hComm, lpBuf, dwCount, &dwBytesRead, NULL))
  {
...
BOOL CSerialPort::PJNRead(void* lpBuf, DWORD dwCount, OVERLAPPED& overlapped)
{
  ASSERT(IsEngrossed());
  ASSERT(m_bOverlapped);
  ...
  DWORD dwBytesRead = 0;
  BOOL bSuccess = ReadFile(port_hComm, lpBuf, dwCount, &dwBytesRead, &overlapped);
  if (!bSuccess)
  {
...
DWORD CSerialPort::PJNWrite(const void* lpBuf, DWORD dwCount)
{
  ASSERT(IsEngrossed());
  ASSERT(!m_bOverlapped);
  
  DWORD dwBytesWritten = 0;
  if (!WriteFile(port_hComm, lpBuf, dwCount, &dwBytesWritten, NULL))
  {
...
BOOL CSerialPort::PJNWrite(const void* lpBuf, DWORD dwCount, OVERLAPPED& overlapped)
{
  ASSERT(IsEngrossed());
  ASSERT(m_bOverlapped);
  ASSERT(overlapped.hEvent);
  ...
  DWORD dwBytesWritten = 0;
  BOOL bSuccess = WriteFile(port_hComm, lpBuf, dwCount, &dwBytesWritten, &overlapped);
  if (!bSuccess)
  {
    if (GetLastError() != ERROR_IO_PENDING)
    {
...
void CSerialPort::GetOverlappedResult(OVERLAPPED& overlapped, DWORD& dwBytesTransferred, BOOL bWait)
{
  ...
  DWORD dwBytesWritten = 0;
  if (!::GetOverlappedResult(port_hComm, &overlapped, &dwBytesTransferred, bWait))
  {
    if (GetLastError() != ERROR_IO_PENDING)
    {
...
void CSerialPort::PJNCancelIo()
{
  ...
  if (!::_SerialPortData.m_lpfnCancelIo(port_hComm))
  {
...
void CSerialPort::PJNWriteEx(const void* lpBuf, DWORD dwCount)
{
  ASSERT(IsEngrossed());
  ...
  if (!WriteFileEx(port_hComm, lpBuf, dwCount, pOverlapped, _OnCompletion))
  {
    delete pOverlapped;
...
void CSerialPort::PJNReadEx(void* lpBuf, DWORD dwCount)
{
  ASSERT(IsEngrossed());
  ...
  ZeroMemory(pOverlapped, sizeof(OVERLAPPED));
  pOverlapped->hEvent = (HANDLE) this;
  if (!ReadFileEx(port_hComm, lpBuf, dwCount, pOverlapped, _OnCompletion))
  {
    delete pOverlapped;
...
void CSerialPort::TransmitChar(char cChar)
{
  ...
  if (!TransmitCommChar(port_hComm, cChar))
  {
...
void CSerialPort::GetConfig(COMMCONFIG& config)
{
  ...
  if (!GetCommConfig(port_hComm, &config, &dwSize))
  {
...
void CSerialPort::SetConfig(COMMCONFIG& config)
{
  ...
  DWORD dwSize = sizeof(COMMCONFIG);
  if (!SetCommConfig(port_hComm, &config, dwSize))
  {
...
void CSerialPort::ClearError(DWORD& dwErrors)
{
  ...
  if (!ClearCommError(port_hComm, &dwErrors, NULL))
  {
...
void CSerialPort::GetStatus(COMSTAT& stat)
{
  ...
  DWORD dwErrors;
  if (!ClearCommError(port_hComm, &dwErrors, &stat))
  {
    TRACE(_T("Failed in call to ClearCommError\n"));
    AfxThrowSerialException();
  }
...
void CSerialPort::GetState(DCB& dcb)
{
  ...
  if (!GetCommState(port_hComm, &dcb))
  {
...
void CSerialPort::SetState(DCB& dcb)
{
  ...
  if (!SetCommState(port_hComm, &dcb))
  {
...
void CSerialPort::Escape(DWORD dwFunc)
{
  ...
  if (!EscapeCommFunction(port_hComm, dwFunc))
  {
...
void CSerialPort::GetProperties(COMMPROP& properties)
{
  ...
  if (!GetCommProperties(port_hComm, &properties))
  {
...
void CSerialPort::GetModemStatus(DWORD& dwModemStatus)
{
  ...
  if (!GetCommModemStatus(port_hComm, &dwModemStatus))
  {
...
void CSerialPort::SetMask(DWORD dwMask)
{
  ...
  if (!SetCommMask(port_hComm, dwMask))
  {
...
void CSerialPort::GetMask(DWORD& dwMask)
{
  ...
  if (!GetCommMask(port_hComm, &dwMask))
  {
...
void CSerialPort::PJNFlush()
{
  ASSERT(IsEngrossed());
  
  if (!FlushFileBuffers(port_hComm))
  {
...
void CSerialPort::PJNPurge(DWORD dwFlags)
{
  ASSERT(IsEngrossed());
  
  if (!PurgeComm(port_hComm, dwFlags))
  {
...
void CSerialPort::Setup(DWORD dwInQueue, DWORD dwOutQueue)
{
  ...
  if (!SetupComm(port_hComm, dwInQueue, dwOutQueue))
  {
...
void CSerialPort::SetTimeouts(COMMTIMEOUTS& timeouts)
{
  ...
  if (!SetCommTimeouts(port_hComm, &timeouts))
  {
...
void CSerialPort::GetTimeouts(COMMTIMEOUTS& timeouts)
{
  ...
  if (!GetCommTimeouts(port_hComm, &timeouts))
  {
...
void CSerialPort::WaitEvent(DWORD& dwMask)
{
  ...
  if (!WaitCommEvent(port_hComm, &dwMask, NULL))
  {
...
void CSerialPort::WaitEvent(DWORD& dwMask, OVERLAPPED& overlapped)
{
  ...
  if (!WaitCommEvent(port_hComm, &dwMask, &overlapped))
  {
    if (GetLastError() != ERROR_IO_PENDING)
    {
      TRACE(_T("Failed in call to WaitCommEvent\n"));
